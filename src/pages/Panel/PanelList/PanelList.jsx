	...
	return (
		<div className="min-w-[350px] bg-white dark:bg-dark">
			<div className="mb-5 mt-6 px-5">
				<h3 className="title">Сообщения</h3>
			</div>
			{isRenderList && (
				<div>
					<div className="px-5">
						...
						<div className="mt-4 flex ml-2">
							<span className="mr-2 text-sm cursor-default opacity-60 text-black dark:text-white">Сортировать по</span>
					...
					<div className="mt-5 scroll overflow-auto h-[calc(100vh-182px)]">
							...
							listChats.map(item => (
								...
								>
									<ItemList
										...
										isCheckFriend={item.message.isCheck}
										isSend={item.message.isSend}
										isCheckMe={item.user.noCheck === 0 ? true : false}
										isOnline={item.friend.isOnline}
										time={item.message.time}
										visibleMessage={item.message.text}
						...
						{notFound && (
							<div className="px-5 flex-center h-[80%]">
								<p className="text-base text-center opacity-60 text-black dark:text-white">Ничего не найдено</p>
							</div>
						)}
					</div>
				</div>
