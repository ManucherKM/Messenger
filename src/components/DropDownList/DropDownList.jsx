		...
		<div className="relative">
			<div className="absolute">
				<div className="text-myblue cursor-pointer hover:opacity-60 flex items-center text-sm">
					<span onClick={uploadVisible} className="mr-2 select-none whitespace-nowrap">
						{selected}
					</span>
				</div>
				{visible && (
						...
						<List
							...
							render={item => (
								<div onClick={() => newSelected(item.text)} key={item.id} className="cursor-pointer hover:opacity-60">
									<div className="p-3">
										<span className="select-none whitespace-nowrap dark:text-white">{item.text}</span>
									</div>
								</div>
							)}
						/>
				...
				{visible && <div onClick={uploadVisible} className="fixed w-[100vw] h-[100vh] top-0 left-0 z-10"></div>}
			</div>
		</div>
	)
}
